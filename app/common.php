 ...
 * 获取excel文件后缀名
 */
function get_excel_name($filename)
{
...
{
    $datas = Db::query('SHOW FULL COLUMNS FROM ' . $table);
    foreach ($datas as $v) {
        $res[$v['Field']] = $v['Comment'];
    }
    return $res;
}
...
function generateRandomString($length = 6)
{
    ...
    if (User::build()->where('invite_code', $randomString)->count()) {
 ...
 * @param $title //标题如系统通知
 * @param $content //说明
 * @param $train_uuid //培训uuid
 * @param $course_uuid //课程uuid
 */
function send_msg($user_uuid, $title, $content, $train_uuid = '', $course_uuid = '')
{
    $msg = new Message();
    $msg->uuid = uuid();
    $msg->user_uuid = $user_uuid;
    $msg->title = $title;
    $msg->train_uuid = $train_uuid;
    $msg->content = $content;
...
function arrayToInsertSql($table, $data)
{
...
function calculateLeftScore(&$currentScore, $scoreChange, $key)
{
    if ($key === 0) {
        $result = $currentScore; // key=0时，先保留原值
        if ($scoreChange > 0) {
            $currentScore -= $scoreChange;
        } else {
            $currentScore += abs($scoreChange);
        }
        return $result;
    } else {
        if ($scoreChange > 0) {
...
function getTreeList($data, $pid = 0)
{
    ...
    foreach ($data as $teamdata) {
        if ($teamdata['pid'] == $pid) {
            $team_data = $teamdata;
            $children_data = getTreeList($data, $teamdata['id']);
...
function post_url($url, $jsonData, $header = '')
{
    try {
        // 初始化cURL会话
        $ch = curl_init();
