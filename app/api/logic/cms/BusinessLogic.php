...
class BusinessLogic
{
    ...
    static public function cmsAdd($request, $userInfo)
    {
        try {
            ...
            AdminLog::build()->add($userInfo['uuid'], '用户管理', '企业管理', '',Business::build()->logData($data));
            return $data['uuid'];
        } catch (Exception $e) {
    ...
    static public function cmsEdit($request, $userInfo, $uuid)
    {
        try {
            ...
            $user->save($request);
            AdminLog::build()->add($userInfo['uuid'], '用户管理', '企业管理', Business::build()->logData($old),Business::build()->logData($user));
            return true;
        } catch (Exception $e) {
            throw new Exception($e->getMessage(), 500);
        }
