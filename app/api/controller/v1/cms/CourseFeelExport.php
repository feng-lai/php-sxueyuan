 ...
 * 心得导出-控制器
 */
class CourseFeelExport extends Api
{
    ...
    public function index()
    {
        ...
        foreach ($result['data'] as $k => $v) {
            if(file_exists(ROOT_PATH . 'public' . DS . $v['avatar']) && $v['avatar']){
                ...
                $resultPHPExcel->getActiveSheet()->setCellValue('A' . $a, $v['avatar']);
            }
            $text = '';
            switch ($v['status']){
                case 0:
            ...
            $resultPHPExcel->getActiveSheet()->setCellValue('G' . $a, $text);
            $a++;
